...
double factor = 1/10.0; // FACTOR = 3 ; BIAS = 1 GIVES CARTOONISH EFFECT
int bias = 0;
int Sharpen[5][5] = {
	                   	{2, 0, 0, 0, 0},
	                   	{0, 2, 0, 0, 0},
...
//	unsigned char b[Pixels*2];
	...
	while (1){
    	...
    	for (ii=2; ii<vHeight-2; ii++)
    	{
        	for (jj=2; jj<vWidth-2; jj++)
        	{
            	red = 0.0;
            	green = 0.0;
            	blue = 0.0;
           		...
           		for (h=-2; h<=2; h++)
            	{
                	for (l=-2; l<=2; l++)
                	{
                    	...
                    	//printf ("  LocnY : %d\n",imageY);
                    	red += r[((ii+h)*vWidth)+(jj+l)] * Sharpen[h+2][l+2];
                    	green += g[((ii+h)*vWidth)+(jj+l)] * Sharpen[h+2][l+2];
                    	blue += b[((ii+h)*vWidth)+(jj+l)] * Sharpen[h+2][l+2];
                	}
            	}
